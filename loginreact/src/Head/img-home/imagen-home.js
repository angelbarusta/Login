...
import user from '../../Generadores/imagenes/atsurabzaid_circle.png';
...
class Imgperfil extends Component{
    ...
    render(){
        if (this.props.nav===false) {
            var ima=(  <Popup content='Hola soy el menu :D' trigger={ <img src={user} alt="Skytsunami" />} />);
        }else{
            var ima=( <AuthUserContext.Consumer>
                {
                  authUser=>(
                    authUser!=null &&
        ...
        <div className="avatar-per"style={{cursor:"pointer"}} onClick={()=>this.handleVer()} >
            <a> 
                {ima}
            </a>
        </div>
